...
@section('content')
    ...
    <div class="container">
        <h3>Giá tiền: {{ $baiviet[0]->gia_tien }} || Diện tích: {{ $baiviet[0]->dien_tich }} mét vuông || Loại:
            @if ($baiviet[0]->type == 0)
                Nhà trọ
            @elseif($baiviet[0]->type == 1)
                Chung cư mini
            @elseif($baiviet[0]->type == 2)
                Nhà riêng
            @endif
        </h3>
        <h3>Địa chỉ: {{ $baiviet[0]->tinhtp }}, {{ $baiviet[0]->quanhuyen }}, {{ $baiviet[0]->phuongxa }} </h3>
        <div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
                @php
                    $imageNames = explode(',', $baiviet[0]->anh_gioi_thieu);
                @endphp
                @foreach ($imageNames as $index => $item)
                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                        <img src="{{ url('public/images') }}/{{ $item }}" class="d-block w-100 img-one"
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}" class="text-reset text-decoration-none">Trang
                        chủ</a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">{{ $baiviet[0]->tieu_de }}</li>
            </ol>
        </nav>
        ...
        <div class="rating">
            <input type="radio" name="rating" value="5" id="star5"
                {{ $baiviet[0]->rate == 5 ? 'checked' : '' }}><label for="star5">☆</label>
            <input type="radio" name="rating" value="4" id="star4"
                {{ $baiviet[0]->rate == 4 ? 'checked' : '' }}><label for="star4">☆</label>
            <input type="radio" name="rating" value="3" id="star3"
                {{ $baiviet[0]->rate == 3 ? 'checked' : '' }}><label for="star3">☆</label>
            <input type="radio" name="rating" value="2" id="star2"
                {{ $baiviet[0]->rate == 2 ? 'checked' : '' }}><label for="star2">☆</label>
            <input type="radio" name="rating" value="1" id="star1"
                {{ $baiviet[0]->rate == 1 ? 'checked' : '' }}><label for="star1">☆</label>
        </div>
        <hr>

        <h5>{{ $baiviet[0]->tieu_de }}</h5>
        {!! $baiviet[0]->noi_dung !!}
    </div>
    ...
    <div class="container">
        ...
        @if (Auth::check())
            ...
            <form action="{{ route('comment.store', $baiviet[0]->id) }}" method="POST">
                @csrf
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".rating label").on("click", function() {
                ...
                var user_id_2 = {{ $baiviet[0]->user_id }};
